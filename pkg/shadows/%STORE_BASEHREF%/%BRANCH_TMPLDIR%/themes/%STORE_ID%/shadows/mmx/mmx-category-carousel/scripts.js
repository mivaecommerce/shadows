	...
	renderCategoryImage(child) {
		...
		if (MMX.isTruthy(child?.image?.custom_image?.settings?.enabled) && child?.image?.custom_image?.image?.image?.length ) {
			...
			if (child?.image?.custom_image?.image?.responsive_images?.mobile) {
				mobileSource = /*html*/`<source class="source__mobile" media="(max-width: 640px)" srcset="${MMX.encodeEntities(MMX.encodeSrcset(child.image.custom_image.image.responsive_images.mobile))}">`;
			}

			if (child?.image?.custom_image?.image?.responsive_images?.tablet) {
				tabletSource = /*html*/`<source class="source__tablet" media="(max-width: 960px)" srcset="${MMX.encodeEntities(MMX.encodeSrcset(child.image.custom_image.image.responsive_images.tablet))}">`;
			}
		}
